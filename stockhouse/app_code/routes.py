...
def shopping_list():
    ...
    debug_print("Suggested Items: ", suggested_items)
...
def add_selected_products():
    try:
        ...
        for item in items:
            ...
            if name and barcode == name:
                ...
                if existing:
                    cursor.execute("""
                        ...
                        SET quantity_to_buy = ?, price = ?, shop = ?, within_budget = 1, insert_date = ?
                        WHERE barcode = ?
                    """, (quantity, price, shop, current_date, manual_barcode))
                    debug_print(f"Aggiornato prodotto manuale '{name}' con barcode '{manual_barcode}'")
                else:
                    cursor.execute("""
                    ...
                    debug_print(f"Inserito prodotto manuale '{name}' con barcode '{manual_barcode}'")
                continue

            cursor.execute("SELECT quantity_to_buy FROM shopping_list WHERE barcode = ?", (barcode,))
            existing = cursor.fetchone()
            ...
            if existing:
                cursor.execute("""
                    ...
                    SET quantity_to_buy = ?, price = ?, shop = ?, within_budget = 1, insert_date = ?
                    WHERE barcode = ?
                """, (quantity, price, row["shop"], current_date, barcode))
                ...
                """, (barcode, row["name"], quantity, row["shop"], reason_to_add, price, current_decade, current_date, within_budget))
        ...
        for item in updated_items:
            ...
            debug_print(f"Ricevuto: {item['quantity_to_buy']} x {item['price']:.2f} → {shop}")
